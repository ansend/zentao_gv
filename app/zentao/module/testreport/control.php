...
class testreport extends control
{
    ...
    public function browse($objectID = 0, $objectType = 'product', $extra = '', $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        ...
        foreach($reports as $report)
        {
            ...
            $products[$report->product] = $report->product;
        }
        ...
        if($products) $products = $this->dao->select('id,name')->from(TABLE_PRODUCT)->where('id')->in($products)->fetchPairs('id', 'name');
    ...
    public function create($objectID, $objectType = 'testtask', $extra = '')
    {
        if($_POST)
        {
       	$reportID = $this->testreport->create();
            //ansen debug for testreport create js::error("hex eror");
        ...
        if($objectType == 'testtask')
        {
            ...
            else
            {
                ...
                //echo js::alert(var_export($build));
                $stories = empty($build->stories) ? array() : $this->story->getByList($build->stories);
		// ansen search stories according to productID
		// echo $productID;
		$stories = $this->story->getProductStories($productID);
		// echo var_export($stories);
