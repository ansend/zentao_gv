...
<table class='table table-condensed table-hover table-striped tablesorter table-fixed table-selectable' id='reportList'>
  ...
  <thead>
    <tr>
      ...
      <?php else:?>
          <th class='w-250px'><?php common::printOrderLink('product',     $orderBy, $vars, $lang->testreport->product);?></th>
      <?php endif;?>
  ...
  <tbody class='text-center'>
    ...
    <tr>
      ...
      <?php else:?>
          <?php $productName = '#' . $report->product . $products[$report->product];?>
	  <td class='text-left' title='<?php echo $productName?>'><?php echo $productName;?></td>
