...
class bug extends control
{
	...
	/*ansen here*/
	/*echo(js::alert($bug->openedBy));
	echo(js::alert($this->app->user->account));*/
	...
	/* Assign. */
	/* add by ansen , only creater can close the bug*/
        if ($bug->openedBy == $this->app->user->account)
        {
            $this->view->isOpener  = "yes";
        }else{
            $this->view->isOpener  = "no";
	}
	/*echo(js::alert($this->view->isOpener)); */
